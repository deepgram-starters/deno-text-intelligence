# deno-text-intelligence

Deno demo app for Deepgram Text Intelligence.
...
| File | Purpose |
|------|---------|
| `server.ts` | Main backend — API endpoints and request handlers |
...
```
feat(deno-text-intelligence): add diarization support
fix(deno-text-intelligence): resolve WebSocket close handling
refactor(deno-text-intelligence): simplify session endpoint
